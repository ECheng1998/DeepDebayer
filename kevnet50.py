...
Original file is located at
    https://colab.research.google.com/drive/1ewkYUF6tyGF7V72S13rZMRH8ChPfXRiU
"""
...
def train_model(train_dl, model, method):
    ...
    if method == 'MSSSIM':
        ...
        perceptual_net = create_perceptualnet().cuda()
        criterion = nn.L1Loss()
        for epoch in range(40):
            for data in train_dl:
                ...
                optimizer.zero_grad()
                outputs = model(temp)
                result = perceptual_net(outputs)
                target = perceptual_net(inputs)
                loss = criterion(result, target)
                loss.backward()
                optimizer.step()
            if ((epoch+1) % 10 == 0):
              print(loss, 'Epochs = ', epoch)
    else:
...
#plt.imshow(outputs[1].permute(1,2,0))

# ------------------ #
#    Testing McM
# ------------------ #

testpath = 'drive/MyDrive/5th Yr Project/TestData/McMaster'
testset = datasets.ImageFolder(root=testpath,transform=preprocess)
...
#PATH = '/content/drive/MyDrive/5th Yr Project/KevNet50MSEs.pt'
...
#for i in range(len(testset)):
#    save_image(images[i], 'Kodak %i.png' % i)
#    save_image(outputs[i], 'Kodako %i.png'% i)